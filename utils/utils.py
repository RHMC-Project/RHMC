...
def RGBtoY(img):
    """
    Convert RGB PIL Image to Y channel (luminance) grayscale image.
    This function is designed to work with torchvision transforms.
    
    Args:
        img: PIL Image in RGB mode
    
    Returns:
        PIL Image: Y channel as grayscale image (mode='L')
    """
    ...
    # Convert to YCbCr and extract Y channel (luminance)
    ycbcr = img.convert('YCbCr')
    y_channel = ycbcr.split()[0]  # Y is the first channel
